...
| 13  | Security Architecture             | Validates the Wrapper Bean pattern for `@RolesAllowed` endpoints, enforces `EJBAccessException`, and proves safe RMI proxy stripping (`LEFT JOIN FETCH`).                                                                            | Successful access / Proxy stripping verified.    | `SupplierIntegrationFacadeBeanIT`    | `testGetActiveOrdersForVendor` / `testPing...` |
| 14  | Timer Services                    | Validates the automated Vendor score calculation (penalizing late deliveries, defects, missing docs) and ensures the timer successfully suspends poorly performing vendors.                                                          | Score calculated accurately, `isEligible=false`. | `SupplierEvaluationTimerBeanIT`      | `testTimerEvaluatesSuppliersAndSuspends`    |

...
| 13  | Timer Services / Transactions     | Validates that the timer correctly reconciles physical counts from the WMS Simulator and isolates restock failures using `REQUIRES_NEW` to prevent transaction rollback of WMS data.                                                 | DB reconciles to WMS count, no rollback.         | `InventoryReplenishmentPollerBeanIT` | `testTimerWMSReconciliation`                |
| 14  | Security Architecture             | Validates the Wrapper Bean pattern for `@RolesAllowed` endpoints, enforces `EJBAccessException`, and proves safe RMI proxy stripping (`LEFT JOIN FETCH`).                                                                            | Successful access / Proxy stripping verified.    | `SupplierIntegrationFacadeBeanIT`    | `testGetActiveOrdersForVendor` / `testPing...` |
| 15  | Timer Services                    | Validates the automated Vendor score calculation (penalizing late deliveries, defects, missing docs) and ensures the timer successfully suspends poorly performing vendors.                                                          | Score calculated accurately, `isEligible=false`. | `SupplierEvaluationTimerBeanIT`      | `testTimerEvaluatesSuppliersAndSuspends`    |
